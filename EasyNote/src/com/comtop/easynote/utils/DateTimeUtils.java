...
public final class DateTimeUtils {
    ...
    public static List<KeyValue> rangeYear(int rangeScope) {
        if (rangeScope <= 1) {
            rangeScope = YEAR_RANGE_SCOPE;
        }
        List<KeyValue> alRangeYearList = new ArrayList<KeyValue>();
        Calendar objCurrentCalendar = Calendar.getInstance();
        int iCurrentYear = objCurrentCalendar.get(Calendar.YEAR);
        int iStartRangeYear = iCurrentYear - rangeScope;
        int iRangeCount = rangeScope * 2 + 1;
        for (int i = 0, j = iRangeCount; i < j; i++) {
            alRangeYearList.add(new KeyValue(String.valueOf(iStartRangeYear + i), String.valueOf(iStartRangeYear + i)));
        }
        return alRangeYearList;
    }

    public static List rangeYear() {
        return rangeYear(YEAR_RANGE_SCOPE);
    }
